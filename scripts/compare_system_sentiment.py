...
while len(samples) < NUM_COMPARISONS:
    ...
    true_follows = ' '.join(true_follows[:NUM_TO_SHOW])
    suggs = {}
    for system_name, flags in systems.items():
        # try:
        ...
        picked_sugg = ' '.join(picked_sugg[:NUM_TO_SHOW])
        suggs[system_name] = picked_sugg
    if any(sugg == true_follows for sugg in suggs.values()):
        continue
    samples.append(dict(
            ...
            true_follows=true_follows,
            sugg=suggs))
    progress.update(1)
    star += 1
    star = star % len(reviews_by_stars)

json.dump(samples, open(paths.parent / 'gruntwork' / f'comparisons_multisys_existing_reviews_{NUM_TO_SHOW}words.json', 'w'), default=lambda x: x.tolist())
